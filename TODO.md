...
- [ ] **ECO-5** Export formats — GeoJSON / KML / GPX; `?screenshot` kiosk + shareable deep-link state — ✅ **GeoJSON + GPX + KML 2026-06-18**: `/data/export.geojson` (RFC 7946 FeatureCollection: Point per fix + LineString per trail) + `/data/export.gpx` (GPX 1.1 wpt/trk) + `/data/export.kml` (OGC KML 2.2 Placemark/Point + LineString), shared `geo_entities()` iterator over aircraft/vessels/beacons, coordinate-order asymmetry (GeoJSON/KML [lon,lat] vs GPX lat/lon attrs) tested. ❌ remaining: `?screenshot` kiosk + deep-link hash state (dashboard.html UI)
