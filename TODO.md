...
- [x] **BENCH-4** ★ Unified benchmark methodology + gap matrix — ✅ **2026-06-19**: added a "Methodology — three gate types" section to `BENCHMARKS.md` (off-air count gate · synthetic AWGN BER floor · field-exact oracle) with a per-mode coverage matrix and the measured sensitivity targets. Makes "what is actually verified" auditable.
...
- [x] **ACARS-6** ★ acarsdec-JSON field-parity vs acarsdec 3.7 — ✅ **2026-06-19**: audited `acarsdec_json.rs` — `msgno` (MIN) + `assstat` already emitted; **added the three missing fields** `sublabel`, `mfi`, and the nested `libacars` app envelope (from `AcarsCore.sublabel/mfi/app`). Now the `:5550` feed carries the full decode for aggregator dedup/reassembly. (extends `ACARS-5.1`/`FEED-2.4`)
- [x] **ADSB-9** ★ readsb `aircraft.json` accuracy-field completeness — ✅ **2026-06-19**: added **nac_v, nic_baro, gva, sda** to `aircraft.json` (contrib from `adsb_status` + the readsb-key copy), all already decoded from TC19/TC29/TC31. `category`/`modeac_count` skipped (no decoded source — emitter-category isn't decoded; Mode A/C demod is unwired per ADSB-11). **receiver.json correction:** `max_range`/`mil` are readsb *stats*, not `receiver.json` fields, and `uuid` is operator-configured (xng has no source) — not invented (skip-don't-fake). (extends `ECO-4`/`ADSB-1`)
...
- [x] **HFDL-7** ★ dumphfdl-compat HFDL position identity — ✅ **2026-06-19**: the **AC-ID→ICAO logon cache** (the valuable half) was already implemented AND applied (`xng-mode-hfdl/src/ac_cache.rs` + `pdu.rs::resolved()` back-fills ICAO into positions before the feed) — audit-confirmed. Added the remaining **`--freq-as-squawk`** as an env-gated opt-in (`XNG_HFDL_FREQ_AS_SQUAWK`, matching the codebase's `XNG_*` flag pattern): stamps the HFDL channel kHz into the Basestation/Beast squawk slot. (extends `HFDL-3`/`HFDL-4`)
...
- [x] **ECO-14** First-party Grafana dashboard — ✅ **2026-06-19**: shipped `docs/grafana/xng-dashboard.json` (6 panels — frame rate, CRC-OK ratio, channel level, FEC rate, top ACARS labels, sample throughput — built on the existing `xng_*` series, templated mode var, importable), referenced from ARCHITECTURE.md. The acarshub `good_loud` counter is deferred as redundant: per-(mode,freq) `xng_frames_crc_ok_total` + `xng_channel_level_dbfs` already give the same "good + loud" insight via a Grafana query. (extends `ECO-7`)
...
- [x] **VERIFY-14** ⚑ NIC-into-position-trust audit — ✅ **2026-06-19**: **no bug.** The local-containment gate (`decode::within_local_containment`, called from `lib.rs:191`) is fed `rc_m = nuc_p_radius_m` (lib.rs:104) — the **NUCp**-derived containment radius (version-agnostic, per-TC), NOT the version-aware NIC. The version-aware NIC is a separate *reported* field and never enters the gate. Finder's worry refuted.
- [x] **VERIFY-15** ⚑ Iridium post-ERA4 audit — ✅ **code correct** (re-verified under `VERIFY-1`/`IRID-8` on 2026-06-18 with ERA4 active: `iri_time_unix_at` selects the newest era-base ≤ ref time; post-2026-01-14 frames decode to 2026; 4 unit tests pin it). A *live-traffic* spot-check (stale-TLE/wrong-year in production logs) ⏸ needs a live Iridium feed not available in-session — queued for the live station.
- [x] **VERIFY-16** ⚑ Serializer-reaches-the-wire audit (= `ADSB-10`) — ✅ **2026-06-19**: traced each flagged ADS-B field decoder→output. **All confirmed SERIALIZED:** TC31 op-status (version/NIC/NACp/SIL/GVA/SDA) + TC29 target-state + DF18 CF-source all reach `adsb_status` JSON (via `frame.rs` tag) → JSONL/aircraft.json/asf-2.0; TC5-8 surface speed/track reach `speed_kt`/`track_deg` → SBS MSG,4 + Beast raw bytes. No serializer stops short. (the finders' "not serialized" suspicions were refuted; `ADSB-9` then surfaced the 4 readsb accuracy fields that *were* on adsb_status but not copied to aircraft.json)
