...
- [ ] **AIS-3** Easy sub-field fills ★ quick win — ✅ types 1-3 ROT (`rot_deg_min` helper), position-accuracy, timestamp, maneuver, RAIM *(done, hand-verified vs the real type-1 vector)*; ❌ remaining: type-5 eta/dte/epfd/dims/ais-version, type-21 name-extension/off_position/virtual/dims/epfd/raim, type 4/11 epfd/raim, type 24B vendor_id/model/serial/dims/mothership, radio comm-state. *(type-5/4 bit positions confirmed vs gpsd; type-18/19/21/24 layouts need per-type verification — the gpsd fast-fetch returned a wrong type-18 timestamp)*
