...
- [ ] **NEW-P2-5** VDES / long-range AIS extensions — ✅ **ASM** via **crate `xng-mode-vdes`**: GMSK 9600 + HDLC/CRC-16 transport on the ASM 1/2 channels (former AIS 27/28), AIS Msg 6/8 ASM headers (source/dest MMSI + DAC/FID), DAC=1 FID 16/31 payloads + raw fallback. Runtime-wired to `--mode vdes` (161.950/162.000 MHz). *(demod synthetic AWGN-BER; sparse public VDES spec — VDE-SAT/terrestrial + more ASM payloads deferred)*; ❌ full AIS-2.0 / VDE data channels still open
