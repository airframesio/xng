...
| airspyone_host / libairspy (Airspy) | BSD-3 | **FFI**: airspy.h API declarations transcribed for the native Airspy R2/Mini backend (xng-sdr `airspy` feature); airspy.c read to confirm arbitrary-rate passthrough (firmware >= 1.0.7) | https://github.com/airspy/airspyone_host |
| airspyhf / libairspyhf (Airspy) | BSD-3 | **FFI**: airspyhf.h API declarations transcribed for the native Airspy HF+/Discovery backend (xng-sdr `airspyhf` feature); bias-tee API avoided (needs >= 1.8, absent from packaged 1.6.x) | https://github.com/airspy/airspyhf |
| iridium-sniffer (alphafox02) | GPL-3 | **Facts only**: ARCHITECTURE.md + iridium.h read for the full pipeline parameters (burst detect, downmix, demod, UW patterns, dqpsk_map, IDA/SBD→ACARS chain) | https://github.com/alphafox02/iridium-sniffer |
...
| Opflasher off-air ACARS + VDL2 captures (Airframes Discord) | contributed for benchmarking, with thanks | Real-RF VHF captures (complex float32). **ACARS** (3.0 MS/s) → acarsdec head-to-head + CI gate (`bench/data/acars_100k.cs16`, real Korean Air HL8537; xng 16 vs acarsdec 3.7 17 clean — comparable). **VDL2** (2.5 MS/s) → dumpvdl2 head-to-head + CI gate (`bench/data/vdl2_opflasher_105k.cs16`, real Air NZ ZK-NNB ADS-C-over-VDL2; xng 13 vs dumpvdl2 2.6.0 12). | Airframes Discord (#contributors) |
