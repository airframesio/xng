//! Off-air validation harness: feed a raw interleaved-f32 IQ capture
//! (e.g. sigidwiki's 21931 kHz HFDL recording, converted with ffmpeg)
//! through the HFDL decoder and print everything that comes out.
//!
//!   ffmpeg -i capture_iq.wav -f f32le -ac 2 -ar 48000 hfdl_48k.f32
//!   cargo run -p xng-mode-hfdl --example offair -- hfdl_48k.f32 48000 0

use num_complex::Complex;
use xng_mode_hfdl::HfdlChannelDecoder;

fn main() {
    let mut args = std::env::args().skip(1);
    let path = args.next().expect("usage: offair <f32le IQ file> <rate> <carrier_offset_hz>");
    let rate: f64 = args.next().expect("rate").parse().expect("rate");
    let offset: f64 = args.next().expect("offset").parse().expect("offset");

    let raw = std::fs::read(&path).expect("read capture");
    let samples: Vec<Complex<f32>> = raw
        .chunks_exact(8)
        .map(|b| {
            Complex::new(
                f32::from_le_bytes([b[0], b[1], b[2], b[3]]),
                f32::from_le_bytes([b[4], b[5], b[6], b[7]]),
            )
        })
        .collect();
    eprintln!(
        "{}: {} IQ samples ({:.1}s at {rate} Hz), carrier offset {offset} Hz",
        path,
        samples.len(),
        samples.len() as f64 / rate
    );

    let mut dec = HfdlChannelDecoder::new(rate, offset).expect("decoder");
    let mut n = 0usize;
    for chunk in samples.chunks(65_536) {
        for e in dec.process(chunk) {
            n += 1;
            match &e.acars {
                Some(b) => println!(
                    "ACARS crc_ok={} tail={:?} label={} text={:?}",
                    b.crc_ok,
                    b.core.tail,
                    b.core.label,
                    b.core.text.chars().take(60).collect::<String>()
                ),
                None => println!("{}: {}", e.kind, e.details),
            }
        }
    }
    println!("total: {n} events");
}
