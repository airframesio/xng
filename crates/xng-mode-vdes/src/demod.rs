//! VDES ASM GMSK demodulator.
//!
//! ITU-R M.2092-1 (the VHF Data Exchange System, "Annex 1 — Technical
//! characteristics for the VHF data exchange system") specifies the ASM
//! (Application-Specific Message) channels as GMSK at 9600 bit/s, modulation
//! index h = 0.5 (±2400 Hz deviation), Gaussian filter BT = 0.5 — the same
//! family as AIS (ITU-R M.1371, BT = 0.4) but with the slightly wider ASM
//! pulse. The link layer (NRZI, HDLC bit-stuffing, 0x7E flags, CRC-16/X-25
//! FCS) is shared with AIS.
//!
//! This is a textbook frequency-discriminator demod (clean-room DSP): per
//! sample frequency discriminator → slow DC tracker (absorbs carrier
//! frequency offset) → per-bit integrate-and-dump with zero-crossing timing
//! recovery → NRZI decode (a transmitted 0 is a level change; a 1 is no
//! change, per M.2092 / M.1371).

use crate::CHANNEL_RATE;
use num_complex::Complex;

/// ASM symbol rate, ITU-R M.2092-1 Annex 1 (ASM): 9600 bit/s.
pub const BAUD: f64 = 9_600.0;
