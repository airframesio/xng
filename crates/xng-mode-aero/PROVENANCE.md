...
- No *physical* AFC of the channel center (JAERO's FreqOffsetEstimateSlot
  also retunes the demodulator); xng channels are DDC-tuned and the
  unlocked-only coarse correction covers reacquisition. The frame-header
  layer of that machine — superframe lock + the DCD/AFC-lock *indicators*
  it drives — is implemented (`state::SuperframeLockStateMachine`, AERO-4,
  detailed below) and surfaced in the message details, even though it does
  not feed back into the channel-center frequency.
...
- The `FrameEncoder` round-trips through `FrameHeader::to_u16` so the wire
  word and the decoder's parse share one definition
  (`frame_header_roundtrips_through_encoder`).

P-channel superframe-lock + DCD/AFC state machine
(`state::SuperframeLockStateMachine`, AERO-4): the consumer of the parsed
header — the xng analogue of JAERO's `FreqOffsetEstimateSlot`, at the
frame-header layer. The low-rate L-band P-channel framer feeds it the
parsed `FrameHeader` of every collected frame; it declares **superframe
lock** after N=3 consecutive in-sequence headers (same `format_id`/
`superframe`, `frame_counter1` advancing by exactly one mod 16, with the
counter wrap 15→0 allowed to step the superframe by one), and drops lock
after M=4 consecutive out-of-sequence headers. A header whose two
redundant counters disagree (`frame_counter1 != frame_counter2`) is
rejected as corrupt and never becomes the comparison baseline. A coupled
`CarrierState` (searching / acquiring / locked) drives the **DCD**
(data-carrier-detect, asserted while acquiring or locked) and
**afc_locked** (held only while locked) indicators — this closes the "No
AFC of the channel center / DCD interplay" divergence above for the
framing layer. The lock snapshot is surfaced as a nested `superframe_lock`
object in the message `details` (enrichment only, no shared-type change);
otherwise-undecoded P-channel events emit a `MessageBody::Aero { kind:
"p-channel-status", details }` carrying `carrier_state`/`dcd`/`afc_locked`
and the in/out-of-sequence run counts. The 10.5k OQPSK and C-band burst
paths use different framers and leave the lock `None` (the state machine
is the low-rate P-channel layer). Validated against a synthetic
frame-counter-sequence oracle
(`frame::tests::superframe_lock_acquires_then_loses_on_synthetic_sequence`
+ the `state` module tests) — legitimate per the hard rules for
state-machine logic: headers are generated by their on-air bit layout,
parsed with the real `FrameHeader::from_soft_bits`, and fed to the real
machine, asserting lock acquires exactly at the N-th in-sequence header,
holds through M-1 misses, drops on the M-th with afc_locked released and
carrier_state→searching.
